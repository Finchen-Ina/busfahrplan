...
SELECT b.buslinienname as Linie, t.gruppe as Tage, lh.haltestellennummer as Haltestellennummer, h.haltestellennamen as Haltestellenname, MINUTE(TIMEDIFF(lh.fahrzeit,Auswahl.fahrzeit)) as Fahrzeit
FROM (SELECT *
	 FROM linienfahrplan_haltestelle
	 WHERE haltestelle_id = 17 /* ID EINTRAGBAR */) AS Auswahl
...
ORDER BY Linie, Tage, Haltestellennummer, Haltestellenname;
...
FROM (SELECT wahl.linienfahrplan_id as linienfahrplan_id, fahrzeit, maxFahrzeit, maxHaltenummer, haltestellennummer
     FROM (SELECT *
	       FROM linienfahrplan_haltestelle
		   WHERE haltestelle_id = 17 /* ID EINTRAGBAR */) AS wahl  
